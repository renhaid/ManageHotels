...
$(function(){
   ...
   $("#grid").datagrid({
       // 设置表格一些属性 
       url : "${pageContext.request.contextPath}/selectSeatInfo" , 
       columns : [[ // 二维数组 支持 多级表头， 每一个数组 就是表头一行
           ...
           {
               field: "seatPeople", // 用于和服务器返回json中字段对应 
               title: "可接受人数", // 列标题显示内容， <th> 
               width: 100
           },
           {
               field: "seatSatus", // 用于和服务器返回json中字段对应 
               title: "餐桌状态", // 列标题显示内容， <th> 
               width: 100
           },
           {
   ...
   //表单验证
  	...
  	$("#btn").click(function(){
  		$.ajax({
  			url:"${pageContext.request.contextPath}/addSeatInfo.do",
  			type:"post",
  			data:{	"seatId":$("#seatId").val(),
  					"seatPeople":$("#select").val(),
  					"seatSatus":$("#seatStatus").val(),
  					"remark":$("#remark").val()	
  			},
  			dataType:"json",
...
function del(id){
	$.messager.confirm('确认','您确认想要删除记录吗？',function(r){    
	    if (r){    
	        $.ajax({
	        	url:"${pageContext.request.contextPath}/delSeatInfo",
	        	type:"post",
	        	data:{"id":id},
	        	dataType:"json",
	        	success:function(data){
...
<div class="place">
    ...
    <div class="formtitle"><span>餐厅信息</span></div>
    	<!--数据表格-->
         <table id="grid"></table>
         ...
         <form 	id="ff" action="addRoomServlet.do" method="post">
         	<table id="table" border="1">
         		<tr><td>餐桌编号:</td><td><input type="text" id="seatId" name="seatId"></td></tr>
         		<tr><td>可接受人数:</td><td><select id="select" name="seatPeople">
         		...
         		<tr><td>状态：</td><td><select id="seatStatus" name="seatSatus">
